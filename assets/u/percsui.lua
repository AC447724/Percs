-- PercsUI v4
-- loadstring(game:HttpGet("https://percs.fun/assets/u/percsui.lua"))()
...
local function Btn(parent, props)
	props.BorderSizePixel=0 props.AutoButtonColor=false
	if not props.Font then props.Font=Enum.Font.GothamMedium end
	if not props.TextSize then props.TextSize=13 end
	if not props.TextColor3 then props.TextColor3=T.Text end
...
function PercsUI:MakeNotification(opts)
	...
	corner(card,6) stroke(card,T.Border)
...
function PercsUI:MakeWindow(opts)
	...
	if doSave then self._saveEnabled=true self._saveFolder=cfgFolder end
	...
	if opts.IntroEnabled ~= false then
		...
		if opts.IntroIcon and opts.IntroIcon~="" then
			New("ImageLabel",{Image=opts.IntroIcon,BackgroundTransparency=1,ImageTransparency=1,
				Size=UDim2.new(0,40,0,40),Position=UDim2.new(0.5,-20,0.5,-38)},bg)
		end

		local il=Lbl(bg,{
			Text=opts.IntroText or title,Font=Enum.Font.GothamBold,TextSize=22,
			TextColor3=T.Accent,TextTransparency=1,
		...
		task.delay(1.3,function()
			tw(bg,{BackgroundTransparency=1},0.35) tw(il,{TextTransparency=1},0.25)
	...
	shadow.BackgroundTransparency=0.75 shadow.ZIndex=0 corner(shadow,12)

	local root=F(sg,T.Win,UDim2.new(0,W,0,H),UDim2.new(0.5,-W/2,0.5,-H/2))
	corner(root,7) stroke(root,T.Border)
	...
	-- left accent bar in topbar
	local tpip=F(topbar,T.Accent,UDim2.new(0,2,0.4,0),UDim2.new(0,12,0.3,0))
	corner(tpip,1)

	-- window icon
	if opts.Icon and opts.Icon~="" then
		New("ImageLabel",{Image=opts.Icon,BackgroundTransparency=1,
			Size=UDim2.new(0,18,0,18),Position=UDim2.new(0,20,0.5,-9)},topbar)
	end

	local titleLbl=Lbl(topbar,{
		Text=title,Font=Enum.Font.GothamBold,TextSize=13,TextColor3=T.Text,
		Size=UDim2.new(1,-100,1,0),
		Position=UDim2.new(0,(opts.Icon and opts.Icon~="") and 44 or 20,0,0),
	})
	...
	local function winBtn(xOff, glyph, hoverCol, cb)
		local b=Btn(topbar,{
			Text=glyph,TextSize=14,Font=Enum.Font.GothamBold,TextColor3=T.Muted,
			BackgroundTransparency=1,Size=UDim2.new(0,26,1,0),Position=UDim2.new(1,xOff,0,0),
		})
	...
	function win:MakeTab(opts)
		...
		-- tab button
		local btn=Btn(self._sidebar,{
			Text=name,TextSize=11,Font=Enum.Font.GothamMedium,TextColor3=T.Muted,
			TextXAlignment=Enum.TextXAlignment.Left,
			BackgroundColor3=T.Element,BackgroundTransparency=1,
			Size=UDim2.new(1,0,0,26),
		})
		corner(btn,4) pad(btn,0,0,8,0)

		-- active indicator pill on left
		local pill=F(btn,T.Accent,UDim2.new(0,2,0.5,0),UDim2.new(0,0,0.25,0))
		pill.Visible=false corner(pill,1)

		-- icon
		if opts.Icon and opts.Icon~="" then
			New("ImageLabel",{Image=opts.Icon,BackgroundTransparency=1,ImageColor3=T.Muted,
				Size=UDim2.new(0,12,0,12),Position=UDim2.new(0,6,0.5,-6)},btn)
		end
		...
		list(page,4) pad(page,10,10,10,10)

		local tab={_btn=btn,_page=page,_pill=pill,_win=self,_lib=self._lib}

		local function activate()
			for _,t in ipairs(self._tabs) do
				t._page.Visible=false t._pill.Visible=false
		...
		-- Rows have a consistent look: dark bg, border, rounded
		local function makeRow(h)
			local r=F(tab._page,T.Element,UDim2.new(1,0,0,h or 32))
			corner(r,5) stroke(r,T.Border)
			return r
		end

		local function rowHover(r)
			r.MouseEnter:Connect(function() tw(r,{BackgroundColor3=T.Hover}) end)
			r.MouseLeave:Connect(function() tw(r,{BackgroundColor3=T.Element}) end)
		end
		...
		function tab:AddSection(opts)
			...
			corner(pip,1)
			Lbl(header,{Text=sName,TextSize=10,TextColor3=T.TextDim,Font=Enum.Font.GothamBold,
				Size=UDim2.new(1,0,1,0),Position=UDim2.new(0,8,0,0)})

			-- inner container for section's elements
			local inner=New("Frame",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,0),
				AutomaticSize=Enum.AutomaticSize.Y,BorderSizePixel=0},self._page)
			list(inner,4)

			-- section proxy — redirect Add* to inner page
			...
			for _,m in ipairs(methods) do
				section[m]=function(s,o)
					local old=proxy._page proxy._page=inner
					local r=proxy[m](proxy,o) proxy._page=old return r
				end
			end
			return section
		end
		...
		function tab:AddButton(opts)
			...
			Lbl(row,{Text=opts.Name or "Button",TextSize=12,TextColor3=T.TextDim,
			...
			hit.MouseEnter:Connect(function()
				...
				local l=row:FindFirstChildOfClass("TextLabel") if l then tw(l,{TextColor3=T.Text}) end
			end)
			hit.MouseLeave:Connect(function()
				tw(row,{BackgroundColor3=T.Element})
				local l=row:FindFirstChildOfClass("TextLabel") if l then tw(l,{TextColor3=T.TextDim}) end
			end)
		...
		function tab:AddToggle(opts)
			...
			Lbl(row,{Text=opts.Name or "Toggle",TextSize=12,TextColor3=T.TextDim,
			...
			-- subtle glow on track when active
		...
		function tab:AddSlider(opts)
			...
			Lbl(row,{Text=opts.Name or "Slider",TextSize=12,TextColor3=T.TextDim,
				Size=UDim2.new(1,-70,0,18),Position=UDim2.new(0,10,0,6)})

			local valLbl=Lbl(row,{
				Text=tostring(val)..(vname~="" and " "..vname or ""),
				TextSize=11,TextColor3=T.Accent,Font=Enum.Font.GothamMedium,
				TextXAlignment=Enum.TextXAlignment.Right,
				Size=UDim2.new(0,62,0,18),Position=UDim2.new(1,-70,0,6),
			})

			-- track (taller = easier to grab)
			...
			function obj:Set(v)
				...
				local p=(v-mn)/(mx-mn)
				valLbl.Text=tostring(v)..(vname~="" and " "..vname or "")
				tw(fill,{Size=UDim2.new(p,0,1,0)}) tw(thumb,{Position=UDim2.new(p,-4.5,0.5,-4.5)})
			end
		...
		function tab:AddDropdown(opts)
			...
			Lbl(row,{Text=opts.Name or "Dropdown",TextSize=12,TextColor3=T.TextDim,
				Size=UDim2.new(0.5,0,1,0),Position=UDim2.new(0,10,0,0)})

			local selLbl=Lbl(row,{Text=sel,TextSize=11,TextColor3=T.Accent,Font=Enum.Font.GothamMedium,
				TextXAlignment=Enum.TextXAlignment.Right,
				Size=UDim2.new(0.44,-20,1,0),Position=UDim2.new(0.5,0,0,0)})

			-- chevron
			local chevron=Lbl(row,{Text="⌄",TextSize=12,TextColor3=T.Muted,TextXAlignment=Enum.TextXAlignment.Center,
				Size=UDim2.new(0,20,1,0),Position=UDim2.new(1,-22,0,0)})

			local obj={Value=sel,_save=opts.Save}
			local sg_=sg

			local dropF=New("Frame",{BackgroundColor3=T.Raised,Size=UDim2.new(0,0,0,0),
				Visible=false,ClipsDescendants=true,ZIndex=40,BorderSizePixel=0},sg_)
			...
			local function rebuildItems(lst,clear)
				...
				for _,opt in ipairs(lst) do
					local item=Btn(dropF,{Text=opt,TextSize=11,TextColor3=T.TextDim,
						TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,
						Size=UDim2.new(1,0,0,itemH),ZIndex=41})
					pad(item,0,0,10,0)
					item.MouseEnter:Connect(function() tw(item,{BackgroundTransparency=0.85,TextColor3=T.Accent}) item.BackgroundColor3=T.Element end)
					item.MouseLeave:Connect(function() tw(item,{BackgroundTransparency=1,TextColor3=T.TextDim}) end)
					item.MouseButton1Click:Connect(function()
						obj.Value=opt selLbl.Text=opt
						tw(dropF,{Size=UDim2.new(0,dropF.Size.X.Offset,0,0)},0.1)
						task.delay(0.11,function() dropF.Visible=false end)
						open=false chevron.Text="⌄"
			...
			row.MouseButton1Click:Connect(function()
				open=not open
				if open then
					local ab=row.AbsolutePosition local sz=row.AbsoluteSize
					...
					tw(dropF,{Size=UDim2.new(0,dropF.Size.X.Offset,0,0)},0.1)
					task.delay(0.11,function() dropF.Visible=false end) chevron.Text="⌄"
				end
			end)
			row.MouseEnter:Connect(function() tw(row,{BackgroundColor3=T.Hover}) end)
			row.MouseLeave:Connect(function() tw(row,{BackgroundColor3=T.Element}) end)

			function obj:Set(v) if table.find(options,v) then obj.Value=v selLbl.Text=v if opts.Callback then opts.Callback(v) end end end
			function obj:Refresh(lst,clear) options=lst totalH=#lst*itemH+4 rebuildItems(lst,clear) end
		...
		function tab:AddTextbox(opts)
			...
			Lbl(row,{Text=opts.Name or "Input",TextSize=10,TextColor3=T.Muted,
				Size=UDim2.new(1,-20,0,14),Position=UDim2.new(0,10,0,5)})

			local box=New("TextBox",{
				BackgroundTransparency=1,
				PlaceholderText=opts.Default or "...",PlaceholderColor3=T.Muted,
				Text="",TextColor3=T.Text,TextSize=12,Font=Enum.Font.GothamMedium,
				TextXAlignment=Enum.TextXAlignment.Left,
				ClearTextOnFocus=opts.TextDisappear or false,
				Size=UDim2.new(1,-20,0,18),Position=UDim2.new(0,10,0,24),
			},row)

			local underline=F(row,T.Border,UDim2.new(1,-20,0,1),UDim2.new(0,10,1,-8))
			box.Focused:Connect(function() tw(row,{BackgroundColor3=T.Hover}) tw(underline,{BackgroundColor3=T.Accent}) end)
			box.FocusLost:Connect(function()
				tw(row,{BackgroundColor3=T.Element}) tw(underline,{BackgroundColor3=T.Border})
		...
		function tab:AddBind(opts)
			...
			local row=F(self._page,T.Element,UDim2.new(1,0,0,32))
			corner(row,5) stroke(row,T.Border)
			Lbl(row,{Text=opts.Name or "Bind",TextSize=12,TextColor3=T.TextDim,
				Size=UDim2.new(1,-90,1,0),Position=UDim2.new(0,10,0,0)})

			local keyBtn=Btn(row,{
				Text=bound==Enum.KeyCode.Unknown and "NONE" or bound.Name,
				TextSize=10,Font=Enum.Font.GothamBold,TextColor3=T.Accent,
				BackgroundColor3=T.Raised,BackgroundTransparency=0,
				Size=UDim2.new(0,72,0,20),Position=UDim2.new(1,-78,0.5,-10),
			})
			corner(keyBtn,4) stroke(keyBtn,T.Border)
			...
			keyBtn.MouseButton1Click:Connect(function()
				if listening then listening=false keyBtn.Text=bound==Enum.KeyCode.Unknown and "NONE" or bound.Name keyBtn.TextColor3=T.Accent return end
				listening=true keyBtn.Text="..." keyBtn.TextColor3=T.Muted
			end)
			UIS.InputBegan:Connect(function(i,gp)
				if gp then return end
				if listening and i.UserInputType==Enum.UserInputType.Keyboard then setKey(i.KeyCode)
				elseif not listening and i.KeyCode==bound and bound~=Enum.KeyCode.Unknown then
					if holdMode then holding=true if opts.Callback then opts.Callback(true) end
					else if opts.Callback then opts.Callback() end end
				end
			end)
			UIS.InputEnded:Connect(function(i)
				if holdMode and i.KeyCode==bound and holding then holding=false if opts.Callback then opts.Callback(false) end end
			end)
		...
		function tab:AddColorpicker(opts)
			...
			local row=F(self._page,T.Element,UDim2.new(1,0,0,32))
			corner(row,5) stroke(row,T.Border)
			Lbl(row,{Text=opts.Name or "Color",TextSize=12,TextColor3=T.TextDim,
				Size=UDim2.new(1,-54,1,0),Position=UDim2.new(0,10,0,0)})

			local swatch=Btn(row,{BackgroundColor3=color,BackgroundTransparency=0,Text="",
				Size=UDim2.new(0,38,0,18),Position=UDim2.new(1,-44,0.5,-9)})
			corner(swatch,4) stroke(swatch,T.Border)

			-- picker popup
			local pickerF=New("Frame",{BackgroundColor3=T.Raised,Size=UDim2.new(0,210,0,168),
				Visible=false,ZIndex=35,BorderSizePixel=0},sg)
			corner(pickerF,6) stroke(pickerF,T.Border)
			...
			local function applyHSV()
				...
				obj.Value=c swatch.BackgroundColor3=c
			...
			local function makeSlider(yPos,label_,gradStart,gradEnd,getV,setV)
				Lbl(pickerF,{Text=label_,TextSize=10,TextColor3=T.Muted,
					Size=UDim2.new(0,12,0,14),Position=UDim2.new(0,0,0,yPos)})
			...
			corner(prev,4) stroke(prev,T.Border) prev.Name="__preview"

			-- hex display
			local hexLbl=New("TextBox",{BackgroundTransparency=1,
				...
				TextXAlignment=Enum.TextXAlignment.Center,
				Size=UDim2.new(1,0,0,16),Position=UDim2.new(0,0,0,92),
				Name="__hex"},pickerF)

			swatch.MouseButton1Click:Connect(function()
				open=not open
				if open then
					local ab=swatch.AbsolutePosition local sz=swatch.AbsoluteSize
					pickerF.Position=UDim2.new(0,math.max(4,ab.X-170),0,ab.Y+sz.Y+4)
					pickerF.Visible=true
				else pickerF.Visible=false end
			end)
		...
		function tab:AddLabel(text,color)
			local l=Lbl(self._page,{Text=text or "",TextSize=11,TextColor3=color or T.Muted,
		...
		function tab:AddParagraph(t_,c_)
			local row=New("Frame",{BackgroundColor3=T.Element,BorderSizePixel=0,
				Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y},self._page)
			corner(row,5) stroke(row,T.Border) pad(row,8,8,10,10)
			local ll=list(row,3)
			local tl=Lbl(row,{Text=t_ or "",Font=Enum.Font.GothamBold,TextSize=12,Size=UDim2.new(1,0,0,16)})
			local cl=Lbl(row,{Text=c_ or "",TextSize=11,TextColor3=T.TextDim,
				Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,TextWrapped=true})
			local obj={} function obj:Set(t,c) tl.Text=t cl.Text=c end return obj
		end
...
-- ─────────────────────────────────────────────────────────────────────────────
function PercsUI:Init() end -- Orion compat stub
